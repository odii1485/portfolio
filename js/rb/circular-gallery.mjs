...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      texture.image = img;
      this.program.uniforms.uImageSizes.value = [img.naturalWidth || 400, img.naturalHeight || 440];
    };
    img.onerror = () => {
      ...
      ctx.strokeStyle = '#cc9b6d';
      ctx.lineWidth = 6;
      ctx.strokeRect(10, 10, 380, 420);
      ctx.fillStyle = '#cc9b6d';
      ctx.font = 'bold 24px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(this.text || 'CERTIFICATION', 200, 220);
      texture.image = cvs;
      this.program.uniforms.uImageSizes.value = [400, 440];
    };
    img.src = this.image;
  }
  createMesh() {
...
export function mountCircularGallery(container, options = {}) {
...
export function initCircularGallery() {
  ...
  const navBtns = gallery.querySelectorAll('.cert-nav');
  ...
  const items = [...gallery.querySelectorAll('.cert-card')].map((card) => {
    const img = card.querySelector('.cert-badge-img');
    const title = card.querySelector('.cert-face.cert-front h4');
    const image = img?.src || '';
    const text = title?.textContent?.trim() || 'Certification';
  ...
  if (linear) {
    linear.hidden = true;
    linear.classList.add('is-rb-hidden');
  }
  navBtns.forEach((b) => { b.hidden = true; });

  gallery.__rbCircularApp = mountCircularGallery(container, {
    items,
    bend: 3,
    textColor: '#e9d391',
    borderRadius: 0.05,
